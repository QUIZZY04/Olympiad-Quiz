 ...
 * @param {string} [entry.language] - Template language code used for this send.
...
/**
 * Writes a single summary document for a completed broadcast/campaign run.
 * If `summary.campaignId` is set (pre-generated by sendBroadcast() so it
 * can be threaded into each recipient's whatsapp_logs row before the
 * summary itself is written), that id becomes the document id - otherwise
 * one is auto-generated exactly as before.
 */
async function logBroadcastSummary(summary) {
  const { campaignId, ...rest } = summary;
  const ref = campaignId
    ? db.collection(COLLECTIONS.BROADCAST_LOGS).doc(campaignId)
    : db.collection(COLLECTIONS.BROADCAST_LOGS).doc();
  await ref.set({ ...rest, campaignId: ref.id, createdAt: FieldValue.serverTimestamp() });
  return ref.id;
}
